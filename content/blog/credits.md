---
date: '2025-11-29T12:00:00.000Z'
title: Credits & Citations
tagline: Giving credit where credit is due
preview: >-
  A comprehensive list of the resources, images, and tools used to build this portfolio and its content.
image: >-
  https://images.unsplash.com/photo-1550684848-fac1c5b4e853?q=80&w=2070
keywords: "Credits, Citations, Resources, Acknowledgements, Assets"
author:
  authorname: Lord Junn
  authorimage: >-
    /ProfilePictures/HomuMado.png
readingtime: 3
---

# Credits & Resources

Building a portfolio is rarely a solo endeavor. It relies on open-source tools, creative assets from the community, and inspiration from others. This page serves to acknowledge and credit the resources used throughout this site.

---

## Image Assets

Most of the photography used for blog headers and project thumbnails comes from talented creators on Unsplash and other stock platforms.

- **Blog Headers:**

- **Icons & Graphics:**

  - Profile Avatar: [Homura x Madoka: Hanaseto_](https://www.weibo.com/7499704029/OC7MOhpmk)

- **Project pics:**
  - Streamlit Food site: [Netflix: Umamusume: Cinderella Gray](https://www.netflix.com/my-en/title/82030222)
---

## Tech Stack & Tools

This site was built using the following technologies:

1.  **Framework:** [Next.js ]
2.  **Styling:** [Tailwind CSS / SCSS / Styled Components]
3.  **Hosting:** [Vercel / Netlify]
4.  **Editor:** Visual Studio Code

---

## Inspirations & References

Design and code structure were inspired by:

- **Design System:** Inspired by [Remind me later]
- **Code Snippets:** Various solutions adapted from StackOverflow and GitHub repositories.

---

## Disclaimer

All external assets are the property of their respective owners. If I have missed a specific credit or if you are the owner of an asset and wish for it to be removed or cited differently, please contact me.

Last updated: 3 Jan 2026